 ...
 * @since 1.0.0
 *
 * @param string $path_to_configuration_file Absolute path to the configuration file's location.
 *
 * @return array|false
 */
function register_shortcode( $path_to_configuration_file ) {

	if ( ! is_readable( $path_to_configuration_file ) ) {
		return false;
	}

	$config = (array) include( $path_to_configuration_file );
	$config = array_merge(
		array(
			'shortcode_name'              => '',
