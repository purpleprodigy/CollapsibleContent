 ...
 *
 * @package     PurpleProdigy\Module\Shortcode
 * @since       1.0.0
...
function render_single_faq( array $attributes, array $config ) {
	...
	if ( ! $faq ) {
		render_none_found_message( $attributes );
...
function render_none_found_message( array $attributes, $is_single_faq = true ) {
	...
	$message = $is_single_faq
		? $attributes['show_none_found_single']
		: $attributes['show_none_found_topic'];
...
function get_shortcode_configuration() {
	return array(
		...
		'defaults' => array(
			...
			'none_found_by_topic'     => __( 'Sorry, no FAQs found for that topic.', FAQ_MODULE_TEXT_DOMAIN ),
			'none_found_by_single'    => __( 'Sorry, no FAQ found.', FAQ_MODULE_TEXT_DOMAIN ),
			""
		),
	);
}
